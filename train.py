...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="configs/h36m/MSTP-b.yaml",
...
def train(args, opts):
    ...
    datareader = DataReaderH36M(n_frames=args.n_frames, sample_stride=1,
                                ...
                                dt_root='data/3d', dt_file=args.dt_file)  # Used for H36m evaluation
...
def floats(args, opts):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = load_model(args).to(device)
    model.eval()  # 必须开启 eval 模式

    # 1. 准备输入 (根据你的实际需求设定输入尺寸)
    input_tensor = torch.randn(1, 243, 17, 3).to(device)
